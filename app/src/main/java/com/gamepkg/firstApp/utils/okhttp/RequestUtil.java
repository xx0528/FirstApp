package com.gamepkg.firstApp.utils.okhttp;
...
class RequestUtil {
    ...
    private RequestBody getRequestBody() {
        ...
        if(!TextUtils.isEmpty(mJsonStr)){
            ...
            return RequestBody.Companion.create(mJsonStr, JSON);//json数据，
        }
    ...
    private void setPostFile() {
        if(mFile != null && mFile.exists()) {
            ...
            RequestBody body = RequestBody.Companion.create(mFile, fileType);//json数据，
    ...
    private void setPostParameAndFile() {
        if(mParamsMap != null && mFile != null){
            ...
            for (String key: mParamsMap.keySet()){
                builder.addFormDataPart(key, Objects.requireNonNull(mParamsMap.get(key)));
            }
            builder.addFormDataPart(mfileKey,mFile.getName(), RequestBody.Companion.create(mFile, MediaType.parse(mFileType)));
    ...
    private void setPostParameAndListFile() {
        if(mfileList != null){
            ...
            for (File f : mfileList){
                builder.addFormDataPart(mfileKey,f.getName(), RequestBody.Companion.create(f, MediaType.parse(mFileType)));
            }
    ...
    private void setPostParameAndMapFile() {
        if(mfileMap != null){
            ...
            for (String key : mfileMap.keySet()){
                builder.addFormDataPart(key, Objects.requireNonNull(mfileMap.get(key)).getName(), RequestBody.Companion.create(Objects.requireNonNull(mfileMap.get(key)), MediaType.parse(mFileType)));
            }
            mRequestBuilder.post(builder.build());
        }
    }
    ...
    private void setHeader() {
        if(mHeaderMap != null){
            for (String key: mHeaderMap.keySet()){
                mRequestBuilder.addHeader(key, Objects.requireNonNull(mHeaderMap.get(key)));
            }
        }
    }
